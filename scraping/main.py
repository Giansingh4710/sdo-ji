...
def i_kirtan():
    #https://www.ikirtan.com/index.php?q=f&f=%2FBhai_Mohinder_Singh_Jee_SDO
    def ikirtan(link,start,end): 
        res=requests.get(link)
        soup=bs(res.text,"lxml")
        atags=soup.find_all("a")
        #in total there are 55 a tags. we want to start at index 5 and end at-3
        links=[]
        for i in range(start,end):
            mp3=link+atags[i].get("href")
            links.append(mp3)
        return links

    links=ikirtan("https://www.ikirtan.com/Bhai_Mohinder_Singh_Jee_SDO/",5,53)
    links+=ikirtan("https://www.ikirtan.com/Bhai_Mohinder_Singh_Jee_SDO/Vancouver_BC_1980s/",5,40)
    path="./ikirtan"
    download(links,path)
